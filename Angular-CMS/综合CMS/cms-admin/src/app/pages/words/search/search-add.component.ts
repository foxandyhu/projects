...
@Component({
  selector: 'ngx-words-search-add',
  templateUrl: './search-add.component.html',
  styleUrls: ['./search-add.component.scss'],
})
export class SearchAddComponent extends BaseComponent implements OnInit {
  ...
  constructor(private searchWordService: SearchWordService,
              protected injector: Injector, private ref: NbDialogRef<SearchAddComponent>) {
    super(null, injector);
  }
  ...
  submit() {
    if (this.isValidForm(this.formId)) {
      this.searchWordService.saveData(this.search).then(() => {
